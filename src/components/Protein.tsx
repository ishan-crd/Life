...
/**
 * Today's protein on the overview. A leaf so logging a gram re-renders this
 * row alone, not the charts and 34 animated bars around it.
 */
export function ProteinToday({ dateKey }: { dateKey: string }) {
  const t = useTheme();
  const entries = useAppStore((s) => s.protein[dateKey]) ?? NONE;
  const goal = useAppStore((s) => s.proteinGoal);
  const openProtein = useProteinSheet(dateKey, 'Today');
  const openGoal = useProteinGoalSheet();
  ...
  return (
    <View style={{ marginTop: 18 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
        <Icon name="bolt" size={14} color={t.inkSoft2} strokeWidth={1.8} />
        <Txt size={typeScale.small} color={t.muted2} style={{ flex: 1 }}>
          Protein today
        </Txt>
        <GoalReadout total={total} goal={goal} onPress={openGoal} />
